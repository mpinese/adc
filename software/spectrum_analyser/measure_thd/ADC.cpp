...
void ADC::send_command(ADC::Command command)
{
...
void ADC::get_samples(uint32_t n_samples, ADC::Channel channel, uint32_t* dest)
{
	assert(this->m_connected);
	...
	while (n_samples_captured < n_samples)
	{
		...
		libusb_status = libusb_bulk_transfer(this->m_bridge_handle, 0x81, this->m_usb_buffer.get(), this->c_buf_size, &n_bytes_received, 1000);
		...
		// uint32_t in dest.
		for (size_t sample_idx = 0; sample_idx < n_samples_in_buf && n_samples_captured < n_samples; sample_idx++)
		{
			dest[n_samples_captured++] =
